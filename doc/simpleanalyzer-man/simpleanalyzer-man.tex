...
\title{\pkname}
\author{Handbuch}
\date{\includegraphics[width=.8\linewidth]{doc_icon.png}}
...
\begin{document}
	...
	\newpage
	\section{Einführung}
	Das \pkname -Softwarepaket enthält Programme zur Auswertung von Versuchen zur Materialerwärmung für debianbasierte Betriebssysteme. 
	Die enthaltene Software ermöglicht es Ihnen, Temperaturmessdaten aus einer .csv-Datei oder Messwerte des ODiSI-Instruments von Luna Inc. in ein einheitliches Format umzuwandeln und zusammenzuführen.\\
	Über eine grafische Oberfläche ist es möglich, mithilfe der so aufbereiteten Daten eine Temperaturverteilung über ein dreidimensionales Modell oder eine Schnittebene zu berechnen sowie den Wärmegehalt zu bestimmen. Sowohl zwei- als auch dreidimensionale Temperaturverteilung können im Programm visualisiert werden. \\
...
\begin{hinweis}
	...
	\begin{sloppypar}
	Benötigen Sie auch Beispieldateien und Quellcode oder existiert kein fertiges Paket für ihr System, müssen Sie alle Dateien aus dem \pkname-Repository herunterladen. Folgen Sie dazu dem Link	\url{https://github.com/vroland/SimpleAnalyzer/archive/master.zip}
	und laden Sie die Zip-Datei herunter. Extrahieren Sie die Datei anschließend mithilfe der Archivverwaltungssoftware ihres Systems in ein Verzeichnis ihrer Wahl. Wenn Sie später auf Beispiele oder Quelltext zurückgreifen wollen, bietet sich hierfür Ihr HOME-Verzeichnis an. Informationen zum Entpacken von Archiven für die meisten debianbasierten Distributionen finden Sie unter \url{http://wiki.ubuntuusers.de/Archivmanager}.
	\end{sloppypar}
	...
	Nun sind alle benötigten Programme installiert.
	\begin{hinweis}
	Wenn sich die Paket-Datei auf einem NTFS- oder FAT-formatierten Datenträger befindet, wird auf manchen Systemen eine Warnung ausgegeben, nach der das Paket von  	\emph{schlechter Qualität} sei. Um dies zu vermeiden, sollten Sie die Datei nicht auf einem solchen Datenträger speichern.
	\end{hinweis}	
	Bei manchen Distributionen genügt es, wenn sie einen Doppelklick auf das Paket durchführen und auf \textbf{[Installieren]}-Button im sich öffnenden Fenster zu klicken (s. Abb. \ref{fig:install_ubuntu}).
	...
	\end{warnung}
	\item Es können beliebig viele zusätzliche Spalten vor und nach den Sensordatenspalten stehen, wie z.B. Namen für die Datensätze. Sie müssen jedoch sicherstellen, dass ihre Konfigurationsdatei (s. \ref{subsec:conf_csvtosd}) dementsprechend angepasst ist.
	...
	Neben der Eingabedatei müssen Sie auch eine Sensordefinitionsdatei angeben, in der Informationen zu den in der Eingabedatei enthaltenen Sensoren abgelegt sind. Die Sensordefinitionsdatei ist eine Textdatei und ist nach folgendem Schema aufgebaut:\\
	\begin{lstlisting}
	name1 x1 y1 z1
	name2 x1 y1 z1
	...
	...
	\end{hinweis}
	Nach einem Sensornamen folgt die Position des Sensors in kartesischen Koordinaten, jeweils durch Leerzeichen getrennt.
	\textbf{Die Sensornamen müssen dabei den Spaltennamen in der umzuwandelnden .csv-Datei entsprechen!}
	\begin{hinweis}
	...
	Zusätzlich sind allgemeine Einstellungen zum Programm in einer Konfigurationsdatei gespeichert. Sie befindet sich, sofern Sie die in \ref{subsec:install} beschriebenen Schritte befolgt haben, unter folgendem Pfad:
	\begin{lstlisting}
	...
	Um Messwerte aus einer ODiSI-Datei in das von \emph{simple\-analyzer-gui} genutzte .tsd-Format (timed sensor data-Format) umzuwandeln, verwenden Sie das Programm \emph{odisitosd}.
	Zusätzlich können Ausreißer in den Messdaten korrigiert werden.
	...
	Dazu wird für jeden Messwert geprüft, ob die Temperaturdifferenz zum vorherigen Wert an dieser Stelle eine bestimmte Schwelle überschreitet. Wenn dies der Fall ist, sucht das Programm unter den nächsten Messwerten einen Wert, für den die Temperaturdifferenz nicht größer als der \textbf{Schwellenwert mal Anzahl der bisherigen Suchschritte} ist.\\
	Wird ein solcher Wert gefunden, werden die dazwischen liegenden Ausreißer-werte \textbf{linear zwischen den beiden gültigen Punkten interpoliert}.
	...
	Neben der Eingabedatei müssen Sie auch hier eine Sensordefinitionsdatei angeben, in der Informationen zum Faserverlauf abgelegt sind. Die Sensordefinitionsdatei ist eine Textdatei und nach folgendem Schema aufgebaut:\\
	\begin{lstlisting}
	#Kommentar
	i l1 x1
	o l2 x2
	...
	\end{warnung}
	Es werden in zwei aufeinander folgenden Zeilen stets Ein- und Ausgangspunkt der Faser beschrieben (durch \emph{i} bzw \emph{o} markiert). $l_1...l_n$ ist dabei die Position auf der Faser, also die Länge des Verlaufs der Faser bis zu diesem Punkt. $x_1...x_n$ ist die Position auf der X-Achse, die Faser liegt dabei immer in X-Z-Ebene. Das für den Versuch verwendete Koordinatensystem muss ggf. entsprechend des Faserverlaufs festgelegt werden.\\
	Optional können Sie für alle Faserdurchläufe durch das Material, die jeweils durch Ein- und Ausgangspunkt eingegrenzt sind, die Richtung des Faserverlaufes festlegen. Fügen Sie dazu eine Zeile hinzu, die mit \emph{d} eingeleitet wird. Nach einem Leerzeichen folgt für jeden Faserdurchlauf dessen Fädelrichtung als positiv (+) oder negativ (-), bezogen auf die Richtung der Z-Achse. Wird die Faser also bspw. abwechselnd von vorn und  hinten durch das Versuchsobjekt gefädelt, ergibt sich auch ein Abwechseln der Faserrichtung (+-+-).
	\subsubsection{Die Konfigurationsdatei}
	...
	\addtocounter{rownum}{1}\arabic{rownum} & 0 & Position auf der X-Achse spiegeln\\
	...
	Zur Auswertung des Versuchs wird auch ein Modell des zu untersuchenden Objekts benötigt. Um dieses zu erstellen können Sie ein beliebiges Werkzeug verwenden, wenn dieses in der Lage ist, Modelle im \textbf{Wavefront (.obj)-Format} zu erstellen. Zur .obj-Datei muss eine Materialbibliotheks-Datei (.mtl) mit Informationen über die verwendeten Materialien vorliegen.\\
	\emph{Objekte mit mehreren Materialien müssen durch mehrere abgeschlossene Teilobjekte mit jeweils eigenem Material erstellt werden.}
	\begin{hinweis}
	Informationen zum Aufbau dieser Dateien finden Sie unter \url{http://en.wikipedia.org/wiki/Wavefront_.obj_file}.
	\end{hinweis}
	\begin{hinweis}
	Über das Attribut \emph{density} in der .mtl-Datei können Sie die  Dichte eines Materials in $\frac{kg}{m^3}$ und mit \emph{spezcap} seine spezifische Wärmekapazität in $\frac{kJ}{kg \cdot K}$ hinterlegen.\\
	...
	\item Drücken Sie nun \textbf{[Shift]} und \textbf{[a]} und navigieren sie in das Untermenü \emph{Mesh}.
	...
	\begin{hinweis}
	Wenn Sie das Hauptmenü von Programmen in Ubuntu 12.04 statt am oberen Bildschirmrand direkt unter der oberen Fensterleiste angezeigt haben möchten, deinstallieren Sie das Paket \emph{indicator-appmenu} und melden Sie sich neu an. Dies deaktiviert jedoch auch das Ubuntu HUD.
	\end{hinweis}
	...
	\item \begin{sloppypar}\textbf{Zeit-bezogene Sensordaten importieren:}\\
	...
	Durch das Klicken auf die Symbole \textbf{6} bis \textbf{8} gelangen Sie zu verschiedenen Analysewerkzeugen:
	\begin{itemize}
	...
	\item \textbf{Automatisch neu berechnen:} Die Temperaturverteilung nach jeder Änderung an den Einstellungen automatisch neu berechnen.
	...
	\item \textbf{Min. Radius-Kantenlänge-Verhältnis:} Minimales Verhältnis $\frac{Umkreisradius}{k\ddot{u}rzeste Kantenl\ddot{a}nge}$ eines Tetraeders.
	...
	\caption{Zeitleiste für einen .tsd-Datensatz}
	...
	Um für die Auswertung relevante Zeitpunkte zu markieren, drücken Sie \textbf{[Analysieren]}. An dieser Stelle wird dadurch eine Markierung gesetzt. Die markierten Zeitpunkte werden in der Analysedatenübersicht (\ref{subsec:data_overview}) angezeigt.
	\begin{itemize}
	\item Klicken Sie die Buttons \textbf{[$<$]} bzw. \textbf{[$>$]} um in die jeweilige Richtung zur nächsten markierten Stelle zu springen.
	...
	\item \textbf{Dichte:} Geben Sie hier die Dichte des Materials mit einem Punkt als Dezimaltrennzeichen in $\frac{kg}{m^3}$ ein.
	\item \textbf{Spez. Wärmekapazität:} Geben Sie hier die spezifische Wärmekapazität der Materials mit einem Punkt als Dezimaltrennzeichen in $\frac{kJ}{kg \cdot K}$ ein.
	\end{itemize}
	\end{itemize}
	\begin{hinweis}
	Wenn eine große Anzahl an Messpunkten vorliegt, kann das Extrapolieren sehr lange dauern. Dies können Sie durch hinzufügen von Messpunkten, die möglichst viel vom Modell umschließen, vermeiden. Dies gilt auch für die Analyse des Modells (s. \ref{subsec:sa_analyze}).
	\end{hinweis}
	...
	\item \textbf{Wert:} Diese Art von Elementen wird der Temperatur entsprechend eingefärbt.
	\end{itemize}
	\item \textbf{zeige extrapolierte Punkte:} Es werden auch Elemente angezeigt, deren Temperatur aus den Messwerten extrapoliert werden musste.
	\item \textbf{zeige Sensordaten:} Die Sensordaten werden als große farbige Punkte im Modell angezeigt.
	\item \textbf{sichtbare Materialien:} Es werden nur die Elemente angezeigt, die zu einem mit einem Haken versehenen Materialien gehören. Möchten Sie ein Material ausblenden, entfernen Sie den Haken vor dem entsprechenden Material.
	...
	Das Programm bietet verschiedene Möglichkeiten zur Analyse der berechneten Temperaturverteilung. 
	...
	\end{figure}
	Navigieren Sie im Hauptmenü zu \textbf{Analysieren - Übersicht} oder wählen Sie das entsprechende Symbol auf der Toolbar (s. \ref{subsec:toolbar_and_loading}), um eine Übersicht über den \textbf{Wärmeenergiegehalt} aller geladenen Objekte angezeigt zu bekommen. Angezeigt werden der \emph{Name des Objekts und Energiegehalt sowie Volumina der einzelnen Materialien und des Gesamtobjekts für alle mit diesem Objekt verknüpften Datensätze}.
	...
	\end{figure}	
	Die Eigenschaften der Ebene und der resultierenden Grafik können Sie im Fenster zur Schnittberechnung einstellen (s. Abb. \ref{fig:sa_render_cut}:
	\begin{itemize}
	...
	\begin{itemize}
	\item \textbf{Breite/Höhe:} Die Auflösung der Grafik.
	...
	\end{itemize}
	Um die \textbf{Position} der Temperaturskala zu ändern, klicken Sie auf das Farbband der Skala, halten sie die \textbf{[linke Maustaste]} gedrückt und ziehen Sie sie an den gewünschten Ort.\\
	...
	\end{itemize}
	Das berechnete Bild wird im unteren Teil des Fensters angezeigt werden. Durch gedrückt halten der \textbf{[linken Maustaste]} und ziehen mit der Maus können Sie die Ansicht auf das Bild verschieben. Mit dem \textbf{[Mausrad]} können Sie hinein und hinaus zoomen.\\
	...
	\end{figure}	
	Wählen Sie \textbf{Datei - Export - Screenshot (Viewport)}, um das Bild, das das 3D-Fenster gerade anzeigt, als Bilddatei zu speichern. Um einen transparenten Hintergrund zu erzeugen, speichern Sie das Bild als .png-Datei.
	...
	\end{figure}	
	Wählen Sie \textbf{Datei - Export - Legacy VTK-Datei}, um das Objekt als Visualisation-Toolkit-Datei zu exportieren. Diese Dateien können in vielen Visualisierungsprogrammen verwendet werden (bspw. \emph{ParaView}).
	\newpage
...
& Stellen Sie auch sicher, dass Sie die Werte mit einem Punkt als Dezimaltrennzeichen eingegeben haben.\\
\vspace{1em}&\vspace{1em}\\

Das Objekt ist in der 3D-Ansicht bei hohem Zoom nur Teilweise zu sehen. & Das Objekt kann nur bis zu einer bestimmten Nähe zur virtuellen Kamera dargestellt werden. Um dennoch Details am Objekt erkennen zu können, verwenden Sie die optische Objektskalierung in den Visualisierungsoptionen. (s. \ref{subsec:sa_viewprops})
\end{tabular}
\newpage
...
This software makes use of version 1.5 of the tetgen library, which is licensed under the terms of the GNU Affero General Public License. (see \url{www.tetgen.org})
