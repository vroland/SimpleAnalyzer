...
\begin{document}
	...
	Die der enthaltene Software ermöglicht es Ihnen, Temperaturmessdaten aus einer .csv-Datei oder Messwerte des ODiSI-Instruments von Luna Inc. in ein einheitliches Format umzuwandeln und zusammenzuführen.\\
...
\begin{hinweis}
	...
	Optional können Sie für alle Faserdurchläufe durch das Material, die jeweils durch Ein- und Ausgangspunkt eingegrenzt sind, die Richtung des Faserverlaufes festlegen. Fügen Sie dazu eine Zeile hinzu, die mit \emph{d} eingeleitet wird. Nach einem Leerzeichen folgt für jeden Faserdurchlauf dessen Fädelrichtung als positiv (+) oder negativ (-), bezogen auf die Richtung der Z-Achse. Wird die Faser also bspw. abwechselnd von vorn und  hinten durch das Versuchsobjekt gefädelt, ergibt sich Abwechseln der Faserrichtung (+-+-).
	\subsubsection{Die Konfigurationsdatei}
	...
	\item \textbf{Spez. Wärmekapazität:} Geben Sie hier die spezifische Wärmekapazität der Materials mit einem Punkt als Dezimaltrennzeichen in $\frac{kJ}{kg*K}$ ein.
	\end{itemize}
	\end{itemize}
	\begin{hinweis}
