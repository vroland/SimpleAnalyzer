...
bool Vector3D::equals(Vector3D* other) {
	return ((abs(getX() - other->getX()) < EPSILON)
			&& (abs(getY() - other->getY()) < EPSILON)
			&& (abs(getZ() - other->getZ()) < EPSILON));
}
...
double Vector3D::getAngleTo(Vector3D* other) {
	double dp = (getX() * other->getX() + getY() * other->getY()
			+ getZ() * other->getZ());
	double multLen = (getLength() * other->getLength());
	double x = dp / multLen;
	if (x < -1.0)
		x = -1.0;	//Rundungsfehler korrigieren
	else if (x > 1.0)
		x = 1.0;
	return acos(x);
}
...
double Vector3D::getDistanceTo(Vector3D* other) {
	return sqrt(
			sqr(getX() - other->getX()) + sqr(getY() - other->getY())
					+ sqr(getZ() - other->getZ()));
}

void Vector3D::print() {
	cout << "Vector3D (" << getX() << " " << getY() << " " << getZ() << ")"
			<< endl;
}
...
void Triangle::print() {
	cout << "Triangle:" << endl;
	cout << "(" << verts[0]->getX() << " " << verts[0]->getY() << " "
			<< verts[0]->getZ() << ")" << endl;
	cout << "(" << verts[1]->getX() << " " << verts[1]->getY() << " "
			<< verts[1]->getZ() << ")" << endl;
...
//Tetrahedron
Tetrahedron::Tetrahedron(Vector3D* v1, Vector3D* v2, Vector3D* v3,
		Vector3D* v4) {
	verts[0] = v1;
	verts[1] = v2;
	verts[2] = v3;
