...
PropertiesBox::PropertiesBox(wxWindow *parent):
	...
	upToDateLbl  = new wxStaticText(parent,wxID_ANY,wxT("Unangewendete\nÄnderungen!"));
	...
	maxVolumeLbl = new wxStaticText(parent,wxID_ANY,wxT("Maximales Tetraedervolumen:"));
	qualityEdit   = new wxTextCtrl(parent,ID_GENERAL_PROP,wxT(""));
	qualityLbl	  = new wxStaticText(parent,wxID_ANY,wxT("Min. Radius-Kantenlänge-Verhältnis:"));
	...
	interpolationModeLbl = new wxStaticText(parent,wxID_ANY,wxT("Interpolationsmodus:"));
	...
	specificHeatCapLbl	  = new wxStaticText(parent,wxID_ANY,wxT("Spez. Wärmekapazität in kJ/(kg*K):"));
	sdTimeline 	  = new GUITimeline(parent,ID_SD_TIMELINE);
	analyzeMarkerCheckBox = new wxCheckBox(parent,ID_ANALYZE_MARKER_CB,wxT("Analysieren"));
	findMaxBt	  = new wxButton(parent,ID_FIND_MAX_BT,wxT("Max"));
	clearAnalyzeMarkerBt = new wxButton(parent,ID_CLEAR_MARKER_BT,wxT("Clear"));
	nextMarkerBt  = new wxButton(parent,ID_MARKER_NEXT_BT,wxT("►"));
	prevMarkerBt  = new wxButton(parent,ID_MARKER_PREV_BT,wxT("◄"));
	autoUpdateCeckBox  = new wxCheckBox(parent,ID_AUTO_UPDATE_CB,wxT("Automatisch neu berechnen"));
