...
DefaultValues::DefaultValues() {
	...
	if (!cfgfile.is_open()) {
		...
		if (!cfgfile.is_open()) {
			cout << "defaults file file simpleanalyzer-gui.conf not found!" << endl;
	...
	getline(cfgfile, line);
	wxGetApp().getVisualizationInfo()->max_visualisation_temp = atof(getTextBlock(line, 0).c_str());
	getline(cfgfile, line);
	wxGetApp().getVisualizationInfo()->min_visualisation_temp = atof(getTextBlock(line, 0).c_str());

	getline(cfgfile, line);
	viewport_showPoints = (Renderer::RenderMode) atoi(getTextBlock(line, 0).c_str());
	getline(cfgfile, line);
	viewport_showEdges = (Renderer::RenderMode) atoi(getTextBlock(line, 0).c_str());
	getline(cfgfile, line);
	viewport_showFaces = (Renderer::RenderMode) atoi(getTextBlock(line, 0).c_str());
