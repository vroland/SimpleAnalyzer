...
/**
 * @brief Namespace des .csv-zu-.tsd-Converterprogramms.
 */
namespace CSVToSD {
	/**
	...
	void parseLine(string line,vector<string> &out,vector<string>* timestamps,vector<string>* names,vector<int>* valid_cols) {
		...
		while (sep_pos!=workstr.npos) {
			...
			if (substr=="") continue;
			if (count>=opts.start_col) {
				if (valid_cols==NULL || contains(*valid_cols,count-opts.start_col)) {
	...
	/**
	 * Liest die Programmargumente, die Konfiguration, die Sensordefinitionsdatei und die .csv-Datei um eine
	 * .tsd-Datei zu generieren, bzw. die Daten der .csv-Datei ein eine .tsd-Datei umzuwandeln.
...
int main(int argc, char *argv[]) {
	return CSVToSD::convert(argc,argv);
}
