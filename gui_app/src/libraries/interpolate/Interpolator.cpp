...
double Interpolator::interpolateTri(Triangle* tri,Vector3D* pos,double* values) {
	...
	/*Dreieck in 2D-Ebene (Bezeichnungen im lokalen Dreieck):
		 *
		 * Das lokale Dreieck ergibt sich durch Transformieren des Dreiecks mit e1 auf x=0 und A auf O(0,0,0).
		 *
		 *				  C			CAB = alpha
		 * 				 /\			ABC = beta
		 * 				/  \		P = pos
	...
	Vector3D localV3 = Vector3D(cos(alpha)*e2->getLength(),sin(alpha)*e2->getLength(),0);
	...
	Vector3D localPos = Vector3D(cos(delta)*v1ToPos->getLength(),ysign*sin(delta)*v1ToPos->getLength(),0);
	Vector3D* v2ToPos = pos->copy();
	v2ToPos->sub(tri->getV2());
	//double gamma = e1->getAngleTo(v2ToPos);
	// Bilineare Interpolation:

	// Punkte auf Kanten 1 und 3:
	Vector3D iPointX1 = Vector3D(localPos.getY()/tan(alpha),localPos.getY(),0);
	Vector3D iPointX2 = Vector3D(localV2.getX()-localPos.getY()/tan(beta),localPos.getY(),0);
	...
	switch (mode) {
		case LINEAR: {
			...
			xVal1 = values[0]+(values[2]-values[0])*getSign(localPos.getY())*iPointX1.getLength()/e2->getLength();
			// analog für Kante e3:
			xVal2 = values[1]+(values[2]-values[1])*getSign(localPos.getY())*sqrt(sqr(iPointX2.getX()-localV2.getX())+sqr(localPos.getY()))/e3->getLength();

			// Interpolieren zwischen Punkten auf Kanten 1 und 3
			value = xVal1+(xVal2-xVal1)*(localPos.getX()-iPointX1.getX())/(iPointX2.getX()-iPointX1.getX());
			break;
		}
		case LOGARITHMIC:
			// siehe Linear
			xVal1 = values[0]*exp((log(values[2])-log(values[0]))*getSign(localPos.getY())*iPointX1.getLength()/e2->getLength());
			xVal2 = values[1]*exp((log(values[2])-log(values[1]))*getSign(localPos.getY())*sqrt(sqr(iPointX2.getX()-localV2.getX())+sqr(localPos.getY()))/e3->getLength());
			// Interpolieren zwischen Punkten auf Kanten 1 und 3
			value = xVal1*exp((log(xVal2)-log(xVal1))*(localPos.getX()-iPointX1.getX())/(iPointX2.getX()-iPointX1.getX()));
			break;
		default:
...
double Interpolator::interpolateTet(Tetrahedron* tet,Vector3D* pos,double* values) {
	//Test auf pos == Eckpunkt
	for (int i=0;i<3;i++) {
	...
	groundVerts[1] = new Vector3D(groundTri.getV1()->getDistanceTo(groundTri.getV2()),0,0);
	groundVerts[2] = new Vector3D(cos(alpha)*e2->getLength(),sin(alpha)*e2->getLength(),0);
	Triangle localGround = Triangle(groundVerts[0],groundVerts[1],groundVerts[2]);
	Vector3D* localTip = new Vector3D(cos(beta)*v1ToV4InGround->getLength(),sign*sin(beta)*v1ToV4InGround->getLength(),h);
	...
	Vector3D* localPos = new Vector3D(cos(pBeta)*posInGround->getLength(),sign*sin(pBeta)*posInGround->getLength(),ph);
	delete lot;
	//Auf Kanten interpolieren
	...
	switch (mode) {
		case LINEAR:
			interValues[0] = values[0]+(values[3]-values[0])*ph/cos(sAlpha)/lengths[0];
			interValues[1] = values[1]+(values[3]-values[1])*ph/cos(sBeta)/lengths[1];
			interValues[2] = values[2]+(values[3]-values[2])*ph/cos(sGamma)/lengths[2];
			break;
		case LOGARITHMIC:
			interValues[0] = values[0]*exp((log(values[3])-log(values[0]))*ph/cos(sAlpha)/lengths[0]);
			interValues[1] = values[1]*exp((log(values[3])-log(values[1]))*ph/cos(sBeta)/lengths[1]);
			interValues[2] = values[2]*exp((log(values[3])-log(values[2]))*ph/cos(sGamma)/lengths[2]);
			break;
		default:
